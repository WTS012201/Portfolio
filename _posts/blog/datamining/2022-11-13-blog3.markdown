...
**Naive Bayes Classifiers** are notorious for performing well in text classification problems. There are several reasons as to why they are well suited for classifying text.
...
To see how the model performs, just total the number of correct predictions from the test size and compared it with the size of the test set to get the accuracy.
...
Since we have six class labels total, it appears to be doing better than just chance. However, this isn't the performance we want. Whenever there's a word that isn't in a class vocabulary from the training data, the posterior probability becomes zero. We could have a pretty good prediction for a class up until there is a word that isn't in the class. To fix this, we can incorporate **Laplace Smoothing** so that we're never eliminating a prediction based on a word not contained in the class.
...
We can see that the model is not performing well for sentences of type `Requirement` and `Skill`. Looking at which words contribute the most to the prediction for the classes may give us some insight into why this is the case.
...
I tried messing around with one of the column values for these words to see if we could improve the performance. I found that if I set some of the words in `Requirements` to one, it improved the model by 3%.
...
I believe this works because it's essentially ignoring the word when computing the **likelihood**. Since we do this for one of the class, the model is able to distinguish between `Requirement` and `Skill` class better.
