...
In this blog, I will be comparing some of the old methods of image classification and comparing them to a more modern model. 
...
**Support Vector Machine** (SVM) is one of the old school methods used for image classification problems. It essentially plots the input feature in a n-dimensional space for two classes and attempts to separate the feature data by finding the hyperplane that best isolates the differences in the data.
...
If you would like to understand more about the learning problem for SVM, I suggest looking at <a href="https://see.stanford.edu/materials/aimlcs229/cs229-notes3.pdfFigure">Andrew Nguyen’s notes on SVM</a>.
...
I resized the image to be 64x64x3 and flattened the image to obtain the feature vector. I'm using the values from the image as the feature input. I have seen more advance techniques used to genereate feature values such as Histogram of Gradients (HOG), but I will be using the more simple approach.
...
ResNet is a Type of CNN that uses a residual learning framework to facilitate the training on substantially deeper NN. It hypothesizes that it is easier to optimize a residual mapping than the unreferenced mapping of the stacked layers.
...
In the paper, they followed every convolution by a batch norm, so I made it its own module.
...
Following the table from above, I constructed the architecture for the ResNet152 they claimed performed the best.
...
Running the model on the dataset with an without normalization.
...
In the paper, they used SGD with weight_decay=0.0001 and momentum=0.9. I used Binary Cross Entropy loss in the for training as that is the standard criterion for image classificaiton for CNNs.
